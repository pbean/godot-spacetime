...
`_schedule_retry_or_disconnect` and `_finish_disconnect`. It snapshots every
pending replacement and pending subscribe, closes their handles, clears the
pending maps and the rest of the session state, restores its reentrance guard,
and only THEN emits the synthetic `subscription_failed` events — **deferred via
`call_deferred` to the next idle frame.** Emitting deferred rather than
synchronously inside the function is a deliberate raise-safety choice:

- **Deferred emission isolates a raising handler.** GDScript has no try/finally. Were the synthetic events emitted synchronously and a connected handler hard-errored (e.g. a null deref), the error would abort the call chain before the guard's `_resetting_subscription_runtime = false` restore ran — wedging the guard `true` and turning every later teardown into the entry-guard early return. Because emission is deferred, the whole teardown (including the guard restore and the caller's state transition) completes first; the handler then runs on a later, isolated call stack where it can neither wedge the guard nor observe a partially-torn-down session. This mirrors the pinned SpacetimeDB 2.1.0 SDK, which dispatches user callbacks on its `FrameTick` queue drain, wraps each in try/catch, and mutates internal state outside that scope so a throw cannot corrupt bookkeeping; it is also consistent with this SDK's already-async reducer-result delivery. One observable consequence: on the full-disconnect path the synthetic `subscription_failed` now fires on the next idle frame — **after** the `state_changed(DISCONNECTED)` and `connection_closed` signals that `_finish_disconnect` emits synchronously. Treat the teardown `subscription_failed` as an asynchronous terminal event, not one ordered before connection close.
- **Entry guard (belt-and-suspenders).** The function starts with `if _resetting_subscription_runtime: return`, sets `_resetting_subscription_runtime = true` before any state mutation, and restores `false` as its last statement. With deferred emission no user code runs inside the function, so the guard is no longer load-bearing for handler raises; it remains as defense against any future synchronous re-entry.
- **Pre-emission snapshots.** Both loops build local arrays (`pending_old_handle_ids` from `_pending_replacements.values()`, `pending_subscription_handles` from `_pending_subscriptions.values()`) before the maps are cleared and before the deferred emit is queued, so a reentrant path that clears those maps cannot cause a queued synthetic emission to be skipped. The pending-subscribe snapshot also validates each value is a live Object with an integer `handle_id`, skipping malformed entries with a `push_warning` so the synchronous guarded region stays raise-proof on internal value shape.
- **Single per-teardown timestamp.** `_reset_subscription_runtime` samples `Time.get_unix_time_from_system()` exactly once — into a local `failed_at_unix_time` read after the guard set-true and before the snapshot loops — and every synthetic `subscription_failed` event from that teardown carries that one value. Two independent reads could skew the events apart, even backwards, under an NTP step or VM suspend. The value is wall-clock unix time (not monotonic ticks), matching `_handle_subscription_error` and the repo-wide event-timestamp convention; switching teardown to `Time.get_ticks_msec()` would corrupt the payload contract by making the same key mean different clocks on different paths.
- **Re-subscribe from a deferred handler is rejected.** Because the events fire after the full teardown, a `subscription_failed` handler that calls `subscribe()` or `replace_subscription()` runs against a terminal session: on the retry path (`_schedule_retry_or_disconnect`) state is `DEGRADED` with the socket disposed; on the disconnect path (`_finish_disconnect`) the `DISCONNECTED` transition has already run. Both entry points reject up front because `_send_subscribe_request` requires `_socket != null` **and** `state == Connected` before `put_packet`, and neither terminal state satisfies that. Callers that need to re-subscribe must wait until the connection returns to `Connected` and re-issue then — this service does not replay subscriptions across a session boundary.
