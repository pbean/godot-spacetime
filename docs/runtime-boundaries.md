...
- **Single per-teardown timestamp.** `_reset_subscription_runtime` samples `Time.get_unix_time_from_system()` exactly once — into a local `failed_at_unix_time` read after the guard set-true and before the snapshot loops — and every synthetic `subscription_failed` event from that teardown carries that one value. This keeps the `failed_at_unix_time` of all events from a single teardown identical; two independent reads (one per emission loop) could skew the events apart, even backwards, under an NTP step or VM suspend between the loops. The value is wall-clock unix time (not monotonic ticks), matching `_handle_subscription_error` and the repo-wide event-timestamp convention; switching teardown to `Time.get_ticks_msec()` would corrupt the payload contract by making the same key mean different clocks on different paths.
- **Synchronous re-subscribe during teardown is rejected.** A `subscription_failed` handler that synchronously calls `subscribe()` mid-teardown cannot leak a re-subscribe onto the outgoing socket. `_send_subscribe_request` requires `_socket != null` **and** `state == Connected` before it calls `put_packet`. The two teardown callers never satisfy both: `_schedule_retry_or_disconnect`'s retry branch has already moved to `DEGRADED` (with a still-non-null socket) before it calls `_reset_subscription_runtime`, and `_finish_disconnect`'s path reaches teardown with `_socket` already null. The public `subscribe()` rejects up front for the same Connected-state reason. Callers that need to re-subscribe must wait until the connection returns to `Connected` and re-issue `subscribe()` then — this service does not replay subscriptions across a session boundary.
- **Synchronous `replace_subscription()` during teardown is also rejected — by a path-specific mechanism, not the same one as `subscribe()`.** On the retry branch (`_schedule_retry_or_disconnect`), state is already `DEGRADED`, so `replace_subscription()` is turned away by its own up-front Connected-state gate before it touches any state. On the disconnect branch (`_finish_disconnect`), `_socket` is disposed before `_reset_subscription_runtime` runs while the `DISCONNECTED` transition is emitted after it — so during synthetic emission, state still reads `Connected` and `_authoritative_handle_id` is not yet cleared. `replace_subscription()` therefore passes its own gates and reaches `_begin_subscription` → `_send_subscribe_request`, which rejects on the null `_socket`; `_begin_subscription`'s send-failure cleanup then erases the transient pending entry. No re-subscribe reaches the wire and nothing survives the teardown. The stopper on this branch is the socket-null gate plus that cleanup — **not** the `_authoritative_handle_id` reset, which only takes effect after the emission loops. As with `subscribe()`, callers must wait for `Connected` and re-issue `replace_subscription()` on the next session.
