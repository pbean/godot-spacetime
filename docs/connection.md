...
- **GDScript native (desktop):** the rule `allow_header_auth := not OS.has_feature("web")` at `addons/godot_spacetime/src/Internal/Platform/GDScript/gdscript_connection_service.gd:542` evaluates `true`, so `build_transport_request` at `addons/godot_spacetime/src/Internal/Platform/GDScript/connection_protocol.gd:70-92` sets `Authorization: Bearer <token>` as a handshake header on `WebSocketPeer`.
