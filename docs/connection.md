...
- **GDScript native (desktop):** the rule `allow_header_auth := not OS.has_feature("web")` at `addons/godot_spacetime/src/Internal/Platform/GDScript/gdscript_connection_service.gd:660` evaluates `true`, so `build_transport_request` at `addons/godot_spacetime/src/Internal/Platform/GDScript/connection_protocol.gd:78-100` sets `Authorization: Bearer <token>` as a handshake header on `WebSocketPeer`.
- **GDScript web export (browser):** the same rule evaluates `false`, so `build_transport_request` at `addons/godot_spacetime/src/Internal/Platform/GDScript/connection_protocol.gd:78-97` appends `?<query_token_key>=<token>` to the WebSocket URL instead. The default parameter name is exposed as `DEFAULT_QUERY_TOKEN_KEY` at `addons/godot_spacetime/src/Internal/Platform/GDScript/connection_protocol.gd:15`.

Non-web deployments that must force query-string transport — for example, reverse proxies that strip `Authorization` headers — can set `prefer_query_token` to `true` and optionally override the parameter name with `query_token_key`; those parameters are declared on `build_transport_request` at `addons/godot_spacetime/src/Internal/Platform/GDScript/connection_protocol.gd:78-84` and threaded through the GDScript runtime options at `addons/godot_spacetime/src/Internal/Platform/GDScript/gdscript_connection_service.gd:248-249` and `addons/godot_spacetime/src/Internal/Platform/GDScript/gdscript_connection_service.gd:661-667`.
