...
| Web export test skips with "non-Mono Godot binary not available for Web export" | Only a `godot-mono` (C#/.NET) Godot binary is on `PATH`. Godot 4 refuses Web export from the C#/.NET editor build even when the staged project itself is pure GDScript — confirmed empirically against `godot-mono 4.6.3` | Install a non-Mono Godot 4.x binary as `godot` or `godot4` on `PATH`, or set `GODOT_WEB_EXPORT_BIN=/path/to/godot` to point the probe at a specific non-Mono binary, then rerun the Story 11.5 browser lane. The Mono-only `probe_godot_binary` keeps serving every other integration test unchanged. |
...
| Browser proof path reaches `phase=query_token_mode_confirmed` but stalls at `connected=false`, the WASM console shows *"The following features required to run Godot projects on the Web are missing: WebGL2"* | Headless Chromium is rendering without a WebGL2-capable backend (commonly seen with `--disable-gpu` or `--headless=new` alone) | The empirically validated headless flag set is pinned as `HEADLESS_BROWSER_FLAGS` in `tests/fixtures/gdscript_web_export.py` — `--headless --no-sandbox --use-angle=swiftshader --enable-unsafe-swiftshader --ignore-gpu-blocklist --run-all-compositor-stages-before-draw`. Confirmed against Chromium 148 + Godot 4.6.3: only the ANGLE swiftshader backend under classic `--headless` boots WebGL2; four other tested combinations stayed at a blank-splash DOM. If a different Chromium major reintroduces the issue, repeat the probe rather than tweaking flags from memory. |
| Browser proof path reaches `phase=query_token_mode_confirmed` but the WebSocket closes immediately with *"HTTP Authentication failed; no valid credentials available"* | A placeholder/static `query_token` was used. SpacetimeDB 2.1.0 validates the ES256 signature on the JWT in `?token=…` during the WebSocket upgrade and rejects anything else with HTTP 401 mid-handshake. | The test fixture mints a fresh anonymous JWT from the live local runtime via `POST <host>/v1/identity` and stages it as `QUERY_TOKEN` — `mint_anonymous_identity_token` in `tests/fixtures/spacetime_runtime.py`. Static fixture tokens cannot be valid across `spacetime start` resets, so the helper is the only safe path. |
| Browser proof path exits in ≈1 s wall-clock with `state={}` (no `data-story-11-5-*` attributes) — even with WebGL2 working and a valid token | The harness is using the legacy `--dump-dom + --virtual-time-budget=…` scheme. Empirically Chromium fast-forwards virtual time when the page is JS-idle and does NOT pause it for WebSocket I/O, so the browser exits before the WebSocket round-trip completes regardless of budget value. | The harness now uses a CDP wait (`--remote-debugging-port=0` + `websocket-client` + `MutationObserver awaitPromise` on `data-story-11-5-phase`) — see `run_headless_browser_capture` in `tests/fixtures/gdscript_web_export.py`. Do NOT reintroduce `--virtual-time-budget` to the flag set; it is fundamentally incompatible with async WebSocket I/O on idle pages. |
| Browser auth mode is reported as `authorization-header` instead of `query-token` | The validation path is not exercising the web-export auth seam correctly | In browser/web mode, validate credentials through the query-token transport path; web exports must not rely on WebSocket handshake headers. See [connection.md → Auth Token Transport](connection.md#auth-token-transport) for the platform-branch rule and override hooks. |
...
| Web export proof | non-Mono Godot 4.x binary + Chromium-family browser (WebGL2-capable via `HEADLESS_BROWSER_FLAGS` — ANGLE swiftshader) + loopback HTTP + web export templates + live local SpacetimeDB runtime (for the runtime-minted JWT via `POST /v1/identity`) | Story 11.5 browser harness |

To unskip the lane locally, satisfy the named prerequisites: start the daemon as shown above, ensure `godot-mono` (or the configured `GODOT_BIN`) is on `PATH`, and — for the Story 11.5 browser lane — install a non-Mono Godot 4.x binary as `godot`/`godot4` (or set `GODOT_WEB_EXPORT_BIN=/path/to/godot`) because Godot 4 refuses Web export from the C#/.NET editor build, install a Chromium-family browser (or set `BROWSER_BIN`), and ensure loopback HTTP binding is permitted. The release-validation GitHub Actions workflow handles the daemon and Godot prerequisites automatically; the browser lane is currently not exercised in CI.
